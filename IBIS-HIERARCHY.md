...
|---|---|---|---|
| 1 (now) | Lightroom-measured tonal order, Source Sans for the interface, hierarchy tables below, solo mode, flat cells with the format badge top-right, hairline module headers, dim level-C icons, dark title bar on Windows, brand strings | about 80 percent | 10 to 12 |
| 2 | one combined toolbar instead of top bar plus filter bar (Ansel-style), Lightroom's nine-control bottom bar, vertical icon rail for the module groups (needs the panel container in `src/gui/gtk.c`, not only `modulegroups.c`), version string out of the top panel | 85 to 90 percent | plus 8 to 10 |
| 3 | rewritten thumbnail cell drawing (filename top-left, stars under the frame, thin selection outline), culling layout chrome, header bar carrying our own controls | 90 to 95 percent | plus 15 to 20 |
