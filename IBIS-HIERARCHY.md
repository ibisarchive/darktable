...
| 2 (started 2026-09-09) | **done:** Spectrum 2 Workflow icons through `dtgtk/icontheme.c` + `themes/icons/spectrum/map.txt` (93 glyphs, chevrons on module headers and panel collapse buttons), version string and brand mark out of the top panel, top bar rebuilt on Lightroom CC (header row: centred search field, icon cluster, view switcher; filter row: rating, colour labels, sort with hairlines; `ui/combined_toolbar` kept as an option), Spectrum grey tokens under the dark theme. **open:**  instead of top bar plus filter bar (Ansel-style), Lightroom's nine-control bottom bar, vertical icon rail for the module groups (needs the panel container in `src/gui/gtk.c`, not only `modulegroups.c`), version string out of the top panel | 85 to 90 percent | plus 8 to 10 |
