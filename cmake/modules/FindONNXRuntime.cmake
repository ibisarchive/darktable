...
if(NOT _ORT_HEADER OR NOT _ORT_LIBRARY)
  ...
  elseif(CMAKE_SYSTEM_NAME STREQUAL "Windows")
    ...
    set(_ORT_NUGET TRUE)
    # MSYS2's cmake can leave CMAKE_SYSTEM_PROCESSOR empty; the host
    # processor and the Windows environment are the next best answers.
    set(_ORT_WIN_ARCH "${CMAKE_SYSTEM_PROCESSOR}")
    if(NOT _ORT_WIN_ARCH)
      set(_ORT_WIN_ARCH "${CMAKE_HOST_SYSTEM_PROCESSOR}")
    endif()
    if(NOT _ORT_WIN_ARCH)
      set(_ORT_WIN_ARCH "$ENV{PROCESSOR_ARCHITECTURE}")
    endif()
    if(NOT _ORT_WIN_ARCH AND CMAKE_SIZEOF_VOID_P EQUAL 8)
      # nothing named the architecture (MSYS2 login shells can drop
      # PROCESSOR_ARCHITECTURE); a 64-bit Windows build is x64 unless told otherwise
      set(_ORT_WIN_ARCH "AMD64")
    endif()
    if(_ORT_WIN_ARCH MATCHES "AMD64|x86_64")
      set(_ORT_NUGET_RID "win-x64")
    elseif(_ORT_WIN_ARCH MATCHES "ARM64|aarch64")
      set(_ORT_NUGET_RID "win-arm64")
    else()
      message(FATAL_ERROR "Unsupported Windows architecture: '${_ORT_WIN_ARCH}'")
    endif()
  else()
  ...
  if(_ORT_NUGET)
    ...
    if(NOT EXISTS "${_ORT_API_JSON}")
      ...
      if(NOT _ORT_API_CODE EQUAL 0)
        list(GET _ORT_API_STATUS 1 _ORT_API_MSG)
        message(WARNING "Failed to fetch release metadata from GitHub: ${_ORT_API_MSG}")
