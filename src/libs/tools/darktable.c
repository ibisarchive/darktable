...
{
  /* Ibis Archive: the window title bar already shows the mark and the name,
     so the panel does not repeat them; the about dialog stays reachable
     from the help menu */
  return DT_VIEW_NONE;
}
                                             ...
                                             dt_lib_module_t *self)
{
  ...
  /* Ibis Archive: the version lives in the about dialog; the top panel only
     shows a workspace label when one is set */
  ...
  if(*wp)
  {
    ...
    pango_layout_set_font_description(layout, font_desc);
    pango_layout_set_text(layout, wp, -1);
